...
function advanced_bootstrap_blocks_block_assets() { // phpcs:ignore
	if (version_compare($GLOBALS["wp_version"], '5.8.0') < 0) {
		$block_category_filter_name = 'block_categories';
	} else {
		$block_category_filter_name = 'block_categories_all'; 
	}
	add_filter( $block_category_filter_name, function( $categories, $post ) {
		return array_merge(
				$categories,
				array(
						array(
